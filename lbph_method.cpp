...
void getMask(const _Image& image, images& transformed_image, const std::string& file_name) {
    ...
    fprintf(new_image, "%d %d\n", image.image_matrix[0].size() - 2, image.image_matrix.size() - 2);
    fprintf(new_image, "255\n");


    // may optimize boundaries to avoid giving _Image image. Need to play with tr_image size
    for (int line = 1; line < image.image_matrix.size() - 1; ++line) { // start from zero and end on size(), if you define pixels out of ranges of image
        for (int column = 1; column < image.image_matrix[line].size() - 1; ++column) {
